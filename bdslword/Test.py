...
def create_main_interface():
    ...
    result_label = tk.Label(root, textvariable=result_text, font=font_large, bg="#f0f0f0")
    result_label.pack(pady=10)

    # Create a label to display the sentence (accumulated words)
    sentence_text.set("বাক্য: ")  # Default text
    sentence_label = tk.Label(root, textvariable=sentence_text, font=font_large, bg="#f0f0f0", wraplength=750)
    sentence_label.pack(pady=10)

    # Create a button to add the predicted word to the sentence
    add_word_button = tk.Button(root, text="বাক্য যোগ করুন", command=add_to_sentence, **btn_style)
    add_word_button.pack(pady=10)

    # Create a button to translate the accumulated sentence
    translate_button = tk.Button(root, text="ইংরেজি", command=translate_sentence, **btn_style)
    translate_button.pack(pady=10)

    # Create a label to display the translation result
    translation_result.set("ইংরেজি অনুবাদ: ")  # Default text
    translation_label = tk.Label(root, textvariable=translation_result, font=font_large, bg="#f0f0f0")
    ...
    predict_button = tk.Button(root, text="চিহ্নিত করুন", command=toggle_prediction, **btn_style)
...
def home_page():
    ...
    welcome_label = tk.Label(root, text="বাংলা সাংকেতিক ভাষা থেকে পাঠ্য এবং ইংরেজি অনুবাদক-এ আপনাকে স্বাগতম", font=('Helvetica', 24), bg="#f0f0f0")
...
btn_style = {'font': font_medium, 'bg': '#4CAF50', 'fg': 'white', 
             'activebackground': '#45a049', 'relief': 'raised', 'padx': 10, 'pady': 5}
